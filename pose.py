...
def get_pose(facial_landmarks, debug_image=None):
    ...
    Returns:
        tuple: pose vector
        numpy array(optional): draw features frame image
    """
    iris_r_center = [0, 0, 0]
    iris_l_center = [0, 0, 0]
    ...
    if debug_image is not None:
        ...
        debug_image = cv2.line(debug_image, [int(iris_l_center[0] * w), int(iris_l_center[1] * h)],
                 [int(iris_r_center[0] * w), int(iris_r_center[1] * h)], (0, 128, 255), 1)
    ...
    if debug_image is not None:
        h, w, c = frame.shape
        # draw iris
        cv2.line(debug_image, [int(iris_l_center[0] * w), int(iris_l_center[1] * h)],
                 [int(iris_l_center[0] * w), int(iris_l_center[1] * h)], (0, 128, 128), 3)
        cv2.line(debug_image, [int(iris_r_center[0] * w), int(iris_r_center[1] * h)],
                 [int(iris_r_center[0] * w), int(iris_r_center[1] * h)], (255, 128, 0), 3)

        # draw eye edge2iris
        debug_image = cv2.line(debug_image, [int(iris_r_center[0] * w), int(iris_r_center[1] * h)],
    ...
    if debug_image is not None:
        return debug_image, (eye_l_h_temp, eye_r_h_temp, mouth_ratio, eye_y_ratio, eye_x_ratio, y_angle, z_angle, x_angle)
    else:
